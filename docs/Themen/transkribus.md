...
Auswahl der Bruchstücke einer Zeile mit Anklicken mit `Cmd` (Mac) oder `Ctrl` (andere Systeme) zur Mehrfachauswahl.
Es macht keinen Unterschied ob man die Regionen der Zeilen (hellblau) oder die Grundlinien (dunkelblau) dafür auswählt.

Wenn die Teile ausgewählt sind, können sie über die Schaltfläche "Merges the selectes spaces" (zusammengeführter Pfeil) verbunden werden.
