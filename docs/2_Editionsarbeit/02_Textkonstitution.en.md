...
The constitution of the text is linked to whether and how the physical **text carriers for transcription are accessible** or can be made accessible. An edition may decide against editing text carriers if they are difficult to access and involve a great deal of (financial, logistical or legal) effort. Text carriers may even already be digitised and available on the database of a memory institution (see below) or [archive.org](https://archive.org/){:target="\_blank"}.

For DSE, further steps are part of the text constitution that decide on the edition of a text carrier, namely clarifications on the **legal modalities** of how the text carriers are made available to the edition. The owners of a text carrier can prohibit a digital, widely accessible publication of the digitised material and/or demand strong copyright restrictions. In extreme cases, this can be a reason to refrain from publishing the text carrier altogether. However, attempts to comply with copyright restrictions can also mean, for example, that the digitised copies of the text carriers are only used in the work process and are not published or that texts and digitised copies are copy-protected.

!!! info "Exemplary DSE"
    The online edition of [Stoffe-Edition](https://fd-stoffe-online.ch/){:target="\_blank"} takes a legally restrictive approach Friedrich Dürrenmatt: The digital copies can be displayed but not downloaded, the transcribed text only allows a copy of a certain number of characters and is not available as TEI/XML. This limited functionality, which deviates from now established DSE data and presentation standards, was analysed from an open access perspective in a [review of the journal RIDE](https://ride.i-d-e.de/issues/issue-17/duerrenmatt/){:target="\_blank"} (A review journal for digital editions and resources). The Stoffe project is an example of a compromise made early in the planning phase: the decision to make a legally protected text accessible at all can be accompanied by major restrictions.
...
!!! note "Experience from the showcase edition"
    ...
    Due to download restrictions in Gallica, the images could not be accessed via the corresponding IIIF, which was not a problem given the size of the project. In the case of larger quantities of images that need to be accessed via IIIF, we recommend contacting the database administration in such cases.
