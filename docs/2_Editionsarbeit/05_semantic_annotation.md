...
!!! warning "Herausforderung"
    Aus editorischer Sicht ist eine Entgrenzung der Kategorien immer mit dem Problem verknüpft, dass eine einmal eingeführte Kategorie sinnvollerweise konsequent angewndet werden muss, was zu viel Aufwand führen kann und im Fall von mehreren Edierenden das genaue Befolgen von komplexen Editionsrichtlinien bzw. ständige Koordination zur ihrer Anpassung bedarf. Komplexe Sachverhalte, die nur in wenigen Texten nachvollziehbar gemacht werden müssen, sollten deshalb eher im Stellen- oder Text-[Kommentar](06_commenting) dargestellt werden. 
...
[Das TEI-Konsortium bietet verschiedene ODDs an und dokumentiert deren Verwendung](https://tei-c.org/guidelines/customization/getting-started-with-p5-odds/); da TEI-XML ein flexibler Standard ist, muss und kann nicht jede Annotation für dasselbe verwendet werden. Im deutschsprachigen Raum hat sich neben den Standard-ODDS des Konsortium das [ODD des deutschen Textarchivs](https://www.deutschestextarchiv.de/doku/basisformat/schema.html) für Edtionen durchgesetzt. ODDs können für jedes Projekt angepasst werden, etwa mithilfe des Online-Tools [ROMA](https://roma.tei-c.org/).
...
Welche der beiden Wege insgesamt weniger Aufwand auf technischer und editorischer Seite benötigt, hängt von der Dynamik eines Projektes ab: Sind die Editionsrichtlinien zur inhaltlichen Annotation einfach und von Anfang an gesetzt, so ist der erste Weg vorzuziehen; ist das Projekt komplex (z.B. durch ein anwachsendes, anfänglich noch schwer abschätzbares Corpus) und die Anforderungen an die Annotationen anfänglich schwer festzulegen, ist die Arbeit im TEI-XML-Code selbst wahrscheinlich sinnvoller. Die längere technische Einarbeitungszeit der Editionsmitarbeitenden zahlt sich dann durch grössere Flexibilität aus. 